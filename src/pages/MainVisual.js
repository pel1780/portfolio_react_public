...
import { BsArrowRightShort, BsChevronLeft, BsPause, BsPlay, BsChevronRight } from "react-icons/bs";
                    ...
                    <div className="left_tab">
                        {
                            travelData.slice(6, 11).map((it, idx) => {
                                return (
                                    <div className={`tit_case ${slideIdx === idx ? 'on' : ''}`} key={it.UC_SEQ}>
                                        <strong>{it.MAIN_TITLE}</strong>
                                        <p>{idx === 4
                                            ? '부산원도심 이바구길 시작점에 서 있는 붉은 벽돌색 건물 하나. 부산최초의 근대식 종합병원'
                                            : it.SUBTITLE}</p>
                                        <Link to={`/detail/${it.UC_SEQ}`}>more
                                            <span><BsArrowRightShort /></span></Link>
                                    </div>
                                )
                            })
                        }
